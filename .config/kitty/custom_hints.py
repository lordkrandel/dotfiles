...
def mark(text, args, Mark, extra_cli_args, *a):
    ...
    for idx, m in enumerate(re.finditer(r'(\/.*[.\w]+)(?::|.*line )(\d{0,4})', text)):
        start, end = m.span()
        mark_text = text[start:end].replace('\n', '').replace('\0', '')
        mark_text = f"{m.group(1)} +{m.group(2)}"
...
def handle_result(args, data, target_window_id, boss, extra_cli_args, *a):
    ...
    for m, g in zip(data['match'], data['groupdicts']):
        if m:
            matches.append(m)
            groupdicts.append(g)
    for path, match_data in zip(matches, groupdicts):
        # Lookup the word in a dictionary, the open_url function
        # will open the provided url in the system browser
        boss.launch('--type', 'overlay-main', *utils.get_editor(), *path.split(' '))
